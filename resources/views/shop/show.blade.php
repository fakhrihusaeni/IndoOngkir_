...
<nav class="flex items-center gap-2 text-sm text-gray-400 mb-6">
    <a href="{{ route('home') }}" class="hover:text-orange-500 transition-colors">Toko</a>
...
<div class="grid lg:grid-cols-2 gap-8 mb-12">
    ...
    <div class="space-y-3">
        ...
        {{-- Image Thumbnails (placeholder untuk multi-image ke depannya) --}}
        <div class="flex gap-2">
            <div class="w-16 h-16 rounded-xl border-2 border-orange-400 overflow-hidden cursor-pointer">
    ...
    <div class="space-y-5">
        ...
        <div>
            <div class="flex items-center gap-2 mb-2">
                <span class="bg-orange-100 text-orange-600 text-xs font-semibold px-3 py-1 rounded-full">Produk UMKM</span>
                @if($product->stock < 10 && $product->stock > 0)
                    <span class="bg-red-100 text-red-600 text-xs font-semibold px-3 py-1 rounded-full">⚡ Stok Terbatas!</span>
                @endif
            </div>
        ...
        <div class="bg-gradient-to-r from-orange-50 to-amber-50 rounded-2xl p-4 border border-orange-100">
            <p class="text-3xl font-bold text-orange-500 font-display">Rp {{ number_format($product->price, 0, ',', '.') }}</p>
            ...
            @if(!auth()->user()->isAdmin())
                ...
                <form method="POST" action="{{ route('cart.add', $product) }}" class="space-y-3">
                    ...
                    <div class="flex gap-3">
                        <button type="submit"
                            class="flex-1 bg-gradient-to-r from-orange-500 to-orange-600 text-white py-3.5 rounded-xl font-semibold hover:shadow-lg hover:shadow-orange-200 transition-all flex items-center justify-center gap-2">
                        ...
                        <button type="button"
                            class="w-14 h-14 border border-gray-200 rounded-xl flex items-center justify-center hover:bg-red-50 hover:border-red-200 transition-all group"
                ...
                @else
                    <div class="bg-red-50 border border-red-200 rounded-xl p-4 text-center">
                        ...
                        <p class="text-red-400 text-xs mt-1">Produk sedang tidak tersedia</p>
                    </div>
                @endif
            @endif
        @else
            <a href="{{ route('login') }}" class="block text-center bg-gradient-to-r from-orange-500 to-orange-600 text-white py-3.5 rounded-xl font-semibold hover:shadow-lg hover:shadow-orange-200 transition-all">
                Login untuk Membeli
            </a>
        @endauth
        ...
        <div class="border border-gray-100 rounded-2xl p-4 space-y-3">
            <div class="flex items-center gap-3 text-sm text-gray-600">
                <div class="w-8 h-8 bg-green-100 rounded-lg flex items-center justify-center flex-shrink-0">✅</div>
                <span>Produk asli 100% dari pengrajin UMKM lokal</span>
            </div>
            <div class="flex items-center gap-3 text-sm text-gray-600">
                <div class="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0">🚚</div>
                <span>Pengiriman JNE, POS, TIKI ke seluruh Indonesia</span>
            </div>
            <div class="flex items-center gap-3 text-sm text-gray-600">
                <div class="w-8 h-8 bg-purple-100 rounded-lg flex items-center justify-center flex-shrink-0">🔒</div>
...
<div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 mb-8">
    ...
    {{-- Rating Summary --}}
    <div class="flex gap-8 mb-6 p-4 bg-orange-50 rounded-2xl">
        <div class="text-center">
            <p class="text-5xl font-bold text-orange-500 font-display">4.8</p>
            ...
            <p class="text-xs text-gray-500">dari 5</p>
        </div>
        <div class="flex-1 space-y-1.5">
            @foreach([5 => 75, 4 => 35, 3 => 12, 2 => 4, 1 => 2] as $star => $count)
            <div class="flex items-center gap-2">
                <span class="text-xs text-gray-500 w-4">{{ $star }}</span>
                <svg class="w-3 h-3 text-yellow-400 fill-current flex-shrink-0" viewBox="0 0 24 24"><path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/></svg>
                <div class="flex-1 bg-gray-200 rounded-full h-1.5">
                    <div class="bg-yellow-400 h-1.5 rounded-full" style="width: {{ ($count/128)*100 }}%"></div>
                </div>
...
<div class="mb-8">
    <div class="flex items-center justify-between mb-5">
        ...
        <a href="{{ route('home') }}" class="text-sm text-orange-500 hover:underline font-medium">Lihat semua →</a>
    </div>
        ...
        <div class="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group">
            ...
            <div class="p-3">
                <a href="{{ route('shop.show', $rec) }}" class="block">
                    <h4 class="font-semibold text-gray-800 text-xs leading-tight hover:text-orange-500 transition-colors line-clamp-2 mb-1">{{ $rec->name }}</h4>
                </a>
                <p class="font-bold text-orange-500 text-sm">Rp {{ number_format($rec->price, 0, ',', '.') }}</p>
                @auth
                    @if(!auth()->user()->isAdmin())
                        <form method="POST" action="{{ route('cart.add', $rec) }}" class="mt-2">
                            @csrf
                            <input type="hidden" name="quantity" value="1">
                            <button type="submit" class="w-full bg-orange-50 text-orange-500 text-xs py-1.5 rounded-lg font-medium hover:bg-orange-500 hover:text-white transition-all">
                                + Keranjang
                            </button>
                        </form>
                    @endif
